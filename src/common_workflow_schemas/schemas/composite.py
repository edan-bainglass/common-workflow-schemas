...
class CompositeInputs(t.Generic[SM]):
    sub_process_class: t.Annotated[
        str,
        MetadataField(
            description="The quantum engine that will be used for the relaxation given as a valid workflow entry point for a common relax implementation. To see a list of all available entry points, use `verdi plugin list aiida.workflows`. Any entry point that starts with `common_workflows.relax.` can be used.",
    ...
    generator_inputs: t.Annotated[
        SM,
        MetadataField(
            description="The inputs to pass to the generator process. If not specified, the default inputs will be used.",
    ...
    sub_process: t.Annotated[
        dict[str, t.Any],
        MetadataField(
            description="Code-dependent inputs overridding those generated by `generator_inputs`. The inputs must be valid ports of the `sub_process_class`.",
...
class CompositeOutputs(SemanticModel):
    ...
    total_magnetizations: t.Annotated[
        ...
        MetadataField(
            description="The total magnetization of the final structure of each sub process, if returned by the underlying common relax workflow.",
            container=list,
        ),
    ]
